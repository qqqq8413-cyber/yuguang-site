...
### 頂欄與手機選單（PR #46 修正）
- 頂欄的排版寫在 `assets/site.css`：桌機用格線 `1fr auto 1fr`，logo 靠左、**導覽列在正中間**、右欄留給手機的選單鈕。以前是 flex 的 `space-between`，靠「← 返回首頁」這個第三個元素把導覽列擠到中間，所以拿掉它就會歪；現在元素多一個少一個都不影響。
...
| #46 | 頂欄一致：拿掉「返回首頁」、導覽列改格線置中、修好常見問題頁被 `.cta` 撞高的頂欄；**補回手機選單鈕**（#42 之後所有主要頁面在手機上沒有選單）；手機可點範圍加大 |
| #47 | 修掉首頁重複的頁尾、產生頁頂欄被 `.cta` 撞高（43 頁）；檢查加入「頁尾只能一個」與 page.css 的 `.cta` |
