...
- 各頁面的 `<style>` **不可以定義沒有前綴的 `.cta`**：導覽列的「聯絡我們」按鈕就是 `a.cta`，會被誤中。常見問題頁曾因此讓頂欄從 65px 變成 198px（頁面自己的 `.cta` 帶了上下 50–80px 的 margin）。頁面自己的按鈕群組請用 `.pagecta`。`check-build.js` 會擋下這兩件事。
